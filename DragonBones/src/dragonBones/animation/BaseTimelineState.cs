...
namespace dragonBones
{
    ...
    abstract public class TweenTimelineState<T, M> : TimelineState<T, M>
        ...
        where M : TimelineData<T>
    {
        public static float _getEasingValue(float progress, float easing)
        {
            if (progress <= 0.0f)
            {
                return 0.0f;
        ...
        public static float _getCurveEasingValue(float progress, List<float> sampling)
        {
            if (progress <= 0.0f)
            {
                return 0.0f;
            ...
            for (int i = 0, l = sampling.Count; i < l; i += 2)
            {
                x = sampling[i];
                y = sampling[i + 1];
                if (x >= progress)
