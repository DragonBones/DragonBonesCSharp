            ...
            foreach (var eventData in frame.events)
            {
                ...
                if (
                    (eventData.type == EventType.Sound ?
                        _armature._eventManager : eventDispatcher
                    )._hasEvent(eventType)
                )
                {
        ...
        override public void update(float time)
        {
            ...
            if (!_isCompleted && _setCurrentTime(time))
            {
                ...
                if (!_isStarted)
                {
                    ...
                    if (eventDispatcher._hasEvent(EventObject.START))
                    {
                        var eventObject = BaseObject.borrowObject<EventObject>();
                        eventObject.animationState = _animationState;
                ...
                {
                    if (eventDispatcher._hasEvent(EventObject.LOOP_COMPLETE))
                    {
                        var eventObject = BaseObject.borrowObject<EventObject>();
                        eventObject.animationState = _animationState;
                    ...
                    if (_isCompleted && eventDispatcher._hasEvent(EventObject.COMPLETE))
                    {
                        var eventObject = BaseObject.borrowObject<EventObject>();
                        eventObject.animationState = _animationState;
