    ...
    public class AnimationState : BaseObject
    {
        ...
        private void _advanceFadeTime(float passedTime)
        {
            ...
            if (_fadeProgress != fadeProgress)
            {
                ...
                var eventDispatcher = _armature._display;

                if (_fadeTime <= passedTime)
                {
                    if (_isFadeOut)
                    {
                        if (eventDispatcher._hasEvent(EventObject.FADE_OUT))
                        {
                    ...
                    {
                        if (eventDispatcher._hasEvent(EventObject.FADE_IN))
                        {
                            var evt = BaseObject.borrowObject<EventObject>();
                            evt.animationState = this;
                ...
                if (_fadeTime >= fadeTotalTime)
                {
                    if (_isFadeOut)
                    {
                        ...
                        if (eventDispatcher._hasEvent(EventObject.FADE_OUT_COMPLETE))
                        {
                            var evt = BaseObject.borrowObject<EventObject>();
                            evt.animationState = this;
                    ...
                    else
                    {
                        ...
                        if (eventDispatcher._hasEvent(EventObject.FADE_IN_COMPLETE))
                        {
                            var evt = BaseObject.borrowObject<EventObject>();
                            evt.animationState = this;
         ...
         * @private
         */
        public bool _isDisabled(Slot slot)
        {
            if (
                displayControl &&
                (
         ...
         * @private
         */
        public void _updateTimelineStates()
        {
            var time = _time;
         ...
         * @private
         */
        public void _updateFFDTimelineStates()
        {
            var time = _time;
            if (!_animationData.hasAsynchronyTimeline)
            {
         ...
         * @private
         */
        public void _advanceTime(float passedTime, float weightLeft, int index)
        {
                ...
                if (isCacheEnabled)
                {
                    ...
                    else
                    {
                        ...
                        if (_armature._animation._animationStateDirty) // Update _cacheFrames.
                        {
                            ...
                            foreach (var boneTimelineState in _boneTimelines)
                            {
                                boneTimelineState.bone._cacheFrames = ((BoneTimelineData)boneTimelineState._timeline).cachedFrames;
                            }

                            foreach (var slotTimelineState in _slotTimelines)
                            {
                                slotTimelineState.slot._cacheFrames = ((SlotTimelineData)slotTimelineState._timeline).cachedFrames;
                            }
                        }
        ...
        {
            if (fadeOutTime < 0.0f || fadeOutTime != fadeOutTime)
            {
                fadeOutTime = 0.0f;
            }
